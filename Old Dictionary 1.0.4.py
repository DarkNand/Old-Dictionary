...
class DictionaryObject(Gtk.Grid):
	...
	def __init__(self, databases, notebook):
		
		Gtk.Grid.__init__(self)
		self.databases = databases
		self.database = self.databases[0]
		
		self.junkWidget = None
		...
		self.radioFrame = Gtk.Frame(label = "Choose Dictionary")
		self.gcideRadio = Gtk.RadioButton("Gcide Dictionary")
		self.gutenbergRadio = Gtk.RadioButton("Gutenberg Dictionary", group=self.gcideRadio)
		self.gcideRadio.set_active(True)
		self.gcideRadio.connect("toggled", self.choose_gcide)
		self.gutenbergRadio.connect("toggled", self.choose_gutenberg)
		...
		self.attach(self.gcideRadio, 0, 4, 1, 1)
		self.attach(self.gutenbergRadio, 1, 4, 1, 1)
		self.attach(self.scroller, 0, 5, 3, 1)

		self.goButton.connect("clicked", self.goButton_callback)
		self.queryEntry.connect("activate", self.goButton_callback)
	
	def choose_gcide(self, widget):
		self.database = self.databases[0]
		self.goButton_callback(self.junkWidget)
		
	def choose_gutenberg(self, widget):
		self.database = self.databases[1]
		self.goButton_callback(self.junkWidget)
	
	def goButton_callback(self, widget):	
		if self.database.name == "gcide":
			self.query = self.queryEntry.get_text()
		elif self.database.name == "gutenberg":
...
class DictionaryWindow(Gtk.Window):
	def __init__(self):	
		#This basically associates the init method with a window
		...
		# Load databases
		self.database = ParseGcide( "gcide.db" )
		self.gutenberg = ParseGutenberg("dictionary.db")
		
		self.databases = [self.database, self.gutenberg]
	...
	def add_tab(self, widget):
		tempTab = DictionaryObject( self.databases, self.notebook )
		self.notebook.append_page(
			tempTab, Gtk.Label( "New Tab" )
		)
